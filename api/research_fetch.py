...
def fetch(url:str):
    ...
    with urllib.request.urlopen(req,timeout=25) as resp:
        ...
        if any(x in ctype.lower() for x in ("text","json","javascript","xml","html")):
            payload=raw.decode(enc,"replace"); mode="text"
        else:
            payload=base64.b64encode(raw).decode("ascii"); mode="base64"
        return {"ok":True,"status":getattr(resp,"status",200),"final_url":resp.geturl(),"content_type":ctype,"content_length":resp.headers.get("Content-Length"),"content_disposition":resp.headers.get("Content-Disposition"),"mode":mode,"truncated":truncated,"payload":payload}
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            qs=urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query); url=(qs.get("url") or [""])[0]
            if not url: return self._send(400,{"ok":False,"error":"url required"})
            self._send(200,fetch(url))
        except Exception as exc: self._send(500,{"ok":False,"error":f"{type(exc).__name__}: {exc}"})
