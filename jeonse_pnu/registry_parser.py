...
# 도로명 내부 공백: "도신로 29길" 형태(로로 끝나는 말 + 공백 + 숫자 + 길)
_RE_ROAD_SPACE = re.compile(r"(\S*로)\s+(\d+)\s*길")
...
def parse_registry_address(raw):
    ...
    # 도로명 내부 공백 정규화: "도신로 29길" → "도신로29길", "국회대로 62길" → "국회대로62길"
    #   ('로'로 끝나는 토큰 + 공백 + 숫자+'길'은 하나의 도로명이다. juso가 이걸 쪼개면 오검색)
    s = _RE_ROAD_SPACE.sub(lambda m: m.group(1) + m.group(2) + "길", s)
